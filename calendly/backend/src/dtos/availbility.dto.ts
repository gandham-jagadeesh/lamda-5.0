...
const availabilityRuleSchema = z.object({
  weekday: z.enum(["sun", "mon", "tue", "wed", "thr", "fri", "sat", "sun"]),
  isActive: z.boolean().default(true),
  startTime: z.string().regex(/^(?:[0-9]|2[0-3]):[0-5][0-9]:[0-5][0-9]$/,{message:"invalid time format Expected HH:MM:SS 24 hours "}),
  endTime: z.string().regex(/^(?:[0-9]|2[0-3]):[0-5][0-9]:[0-5][0-9]$/, { message: "invalid time format Expected HH:MM:SS 24 hours " }),
...
const availabilityExceptionSchema = z.object({
  ...
  startTime: z.string().regex(/^(?:[0-9]|2[0-3]):[0-5][0-9]:[0-5][0-9]$/, { message: "invalid time format Expected HH:MM:SS 24 hours " }).optional(),
  endTime: z.string().regex(/^(?:[0-9]|2[0-3]):[0-5][0-9]:[0-5][0-9]$/, { message: "invalid time format Expected HH:MM:SS 24 hours " }).optional(),
  timeZone: z.string().default("UTC").optional(),
